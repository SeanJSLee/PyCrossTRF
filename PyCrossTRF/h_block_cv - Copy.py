...
class CV_h_block:
    ...
    def __init__(self, df: pd.DataFrame, dep: str, indep: str, pq_order: dict, date_var = None, cov_scale:dict = None):
        """
        ...
        if date_var is not None :
            df = df.sort_values(by=date_var).reset_index(drop=True).copy()
            self.df_date = df[[date_var]].drop_duplicates().reset_index(drop=True)
        else :
            df = df.copy()
