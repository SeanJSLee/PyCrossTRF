...
class Normalizer:
    def __init__(self, 
                 ctrf_model:Optional = None # type: ignore
                 ) -> None:
        if ctrf_model  :
    ...
    def normalizer( self,
                    ...
                    trans_var: Optional[str] = None,
                    method: Literal['raw','minmax','linear','time', 'standard','quantile'] = 'raw',
        ...
        if method == 'raw':
            xs = df[trans_var] # type: ignore

        elif method in(['minmax','linear','standard','quantile']):
            return apply_transform(df=df,  # type: ignore
                                    trans_var=trans_var,  # type: ignore
                                    method=method,  # type: ignore
                                    cross_id=cross_id) # type: ignore
    ...
    def centering_cov(self,
                      ...
                      log_converted: bool,
                      cross_id:Optional[str] = None,
                      method: Literal['raw','standard'] = 'standard',
        ...
        * calculate covariates' value at the 'MMT'
            * e.g. 25 Celsius degree is MMT, calculate inverse weight (distance between months' average temp) 
                    of covariate. Once obtain weighted average, shift covariate's value by the average.
                * note:
                    TRF draws 'outcome' variable response decomposed by 'frequency' 
                    which covariates may not the same frequency, furthermore, given time unit distribution is unkown.
                    Therefore, 'centering' is not perfect but 'mmt_s' is monthly summarized value.
            * cautious for log transformed covariates.
            * 'trend' does not need centering or standardization.
        '''
        # df = self.df
        
        if cross_id is None :
            shift : float           # type: ignore
            wgt:pd.Series = (1 / abs(s - mmt_s))                       # pyright: ignore[reportOptionalOperand]
            if not log_converted:
                shift = float(np.average(df[trans_var], weights=wgt) - df[trans_var].mean()) # type: ignore
            else :
                shift = float(np.average(np.exp(df[trans_var]), weights=wgt)) - np.exp(df[trans_var]).mean()
            df[f'{trans_var}_centered'] = df[trans_var] - shift             # type: ignore
            df[f'{trans_var}_shiftscale'] = shift
            # 
            df = pd.concat([df, apply_transform(df = df,
                             trans_var=trans_var,
                             method='standard',
                             mmt_s=mmt_s,
                             centering='_centered').rename(columns={f'{trans_var}_{method}':f'{trans_var}_{method}_centered'})
                             ],axis=1)
            return df
            ...
            shift : pd.Series
        # need converter to compensate centering <= call transformer add centering value.
                                  # type: ignore
...
def apply_transform(df:pd.DataFrame,
                     trans_var:str,
                     method:Literal['minmax','linear','standard','quantile'], 
                     cross_id:Optional[str] = None,
                     mmt_s : Optional[float] = None,
                     xs    : Optional[pd.Series] = None,
                     centering : Literal['','_centered'] = ''
    ...
    if cross_id is None :
        ...
        # 
        return pd.DataFrame({f'{trans_var}_{method}{centering}_transformer':[transformer] * len(df),
                                    f'{trans_var}_{method}':transformed}, 
                                    index=df.index) 
    # 
    elif cross_id :
        # 
        def _transform_cx(g : pd.DataFrame, method=method, trans_var=trans_var, cross_id=cross_id):
            # 
            if (method == 'minmax') or (method == 'linear'):
                transformer = MinMaxScaler()
            elif method == 'standard':
            ...
            return pd.DataFrame({f'{trans_var}_{method}_transformer_{cross_id}':[transformer] * len(g), # type: ignore
                                    ...
                                    index=g.index)
        # 
        return df[[trans_var,cross_id]].groupby(cross_id, observed=True, group_keys=False).apply(_transform_cx)
    


def pq_powering(temp_s : pd.Series, 
                pq_order : dict = {'p':4, 'q':1},  
                var_name:Optional[str]=None, 
                covariate_s:Optional[pd.Series] = None, 
                verbose:bool    =False):
        '''
        tranform 'temp_s' based on 'p', 'q' order.
        if 'covaraite_s' provided, multiplying it.
...
def gen_df_xs(temp_s : pd.Series, 
              pq_order : dict = {'p':4, 'q':1}, 
              xs_cov : Optional[pd.DataFrame] = None, 
              verbose:bool = False) :
    """
    Generate pq powered df based on temp and covariates.
    """
    df = pq_powering(temp_s=temp_s, pq_order=pq_order)
    if xs_cov is not None :
        for cov in xs_cov.columns : 
            df = pd.concat([df, pq_powering(temp_s=temp_s, pq_order=pq_order, covariate_s=xs_cov[[cov]])], axis=1)
    if verbose : print(df)
