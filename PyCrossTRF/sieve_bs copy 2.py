...
class SieveBootstrap:
    ...
    # main process.
    def sieve_bootstrap_i(self, cross_id:str, save_results:bool = False, use_gpu:bool=False, device='cuda'):
        '''
        ...
        for idx in df_y_star_B.index:
            tau025, tau975 = self.find_bootstrap_tails(idx, df_y_star_B, use_gpu=use_gpu, device=device)
    ...
    def find_bootstrap_tails(self, idx, df_y_star_B, use_gpu:bool=False, device='cuda'):            
        ...
        # 
        if use_gpu:
            # Send data to GPU as a tensor.
            boot_y_tensor = torch.tensor(bootstrapped_y_star_at_T, device=device, dtype=torch.float32)
            # Compute the 2.5th and 97.5th percentiles
            tau025 = torch.quantile(boot_y_tensor, 0.025).item()
            tau975 = torch.quantile(boot_y_tensor, 0.975).item()
        else:
